...
class WebScraper:
    ...
    def scrape_page(self, page_number):
        ...
        for element in elements:
            article = {
                "Meta": element.find_next(class_=self.meta_class).get_text(strip=True) if element.find_next(class_=self.meta_class) else "",
                "Title": element.find_next(class_=self.title_class).get_text(strip=True) if element.find_next(class_=self.title_class) else "",
                "Tag": element.find_next(class_=self.tags_class).get_text(strip=True) if element.find_next(class_=self.tags_class) else "",
                "Properties": element.find_next(class_=self.properties_class).get_text(strip=True) if element.find_next(class_=self.properties_class) else "",
                "Price-Tag": element.find_next(class_=self.price_class).get_text(strip=True) if element.find_next(class_=self.price_class) else ""
            }
